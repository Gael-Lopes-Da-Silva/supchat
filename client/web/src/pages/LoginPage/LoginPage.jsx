...
const LoginPage = () => {
  ...
  useEffect(() => {
    ...
    if (token) {
      try {
        ...
        localStorage.setItem(
          "user",
          JSON.stringify({ token, data: decodedToken })
        );
    ...
    if (isConfirmed) {
      toast.info(
        "Votre compte a été confirmé. Vous pouvez maintenant vous authentifier.",
        { position: "top-center" }
      );
    }

    if (isPasswordReseted) {
      toast.info("Votre nouveau mot de passe a été enregistré.", {
        position: "top-center",
      });
    }
  ...
  const handleGoogle = () => {
    // VEUILLEZ ME MP SUR DISCORD OU TEAMS POUR LE .ENV (zak)
