        ...
        try {
          ...
          if (existingUser.length > 0) {
            ...
            if (userResult.length === 0) {
              return done(
                new Error("Utilisateur introuvable après insertion...")
              );
            }
    ...
    async (req, accessToken, refreshToken, profile, done) => {
      try {
        ...
        try {
          ...
          if (existingFacebookAccount.length > 0) {
            ...
            if (linkedLocalAccount.length > 0 && linkedUserId !== userId) {
              ...
              return req.res.redirect(
                `http://localhost:5000/settings?error=Ce compte Facebook est déjà lié à un autre compte local.`
              );
            }
          }
          ...
          // Ici au moment où je vois qu'il y a effectivement le provider facebook dans la table des providers je lui fou la même user id que le compte
          //  qu'on lie et je stocke l'id ACTUEL de la ligne pas encore updaté dans original_user_id. Pourquoi me direz vous..
          //  Eh bien si un jour un user veut délier son compte il faudra re update cette meme ligne et
          //  remettre l user id original pour rétablir une bonne liaison entre la table users et providers.
          //  Ca permet entre autre de pouvoir se reconnecter à son compte facebook si on l'avait creer avant de le lier.. Un peu casse tête tout ça mais tkt..
          if (existingEntry.length > 0) {
            ...
            if (updateResult.affectedRows === 0) {
              await connection.rollback();
              return req.res.redirect(
                `http://localhost:5000/settings?error=Erreur lors de la fusion des comptes.`
              );
            }
          } else {
            await connection.query(
